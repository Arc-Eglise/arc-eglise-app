-- ═══════════════════════════════════════════════════════════════
--  ARC — Témoignages
--  À exécuter : Supabase > SQL Editor > New Query
-- ═══════════════════════════════════════════════════════════════

CREATE TABLE IF NOT EXISTS testimonials (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  author_name  TEXT NOT NULL,
  author_role  TEXT,                          -- "Membre depuis 2022", "Visiteur"
...
CREATE POLICY "CMS — gérer témoignages"
  ...
  USING (
    EXISTS (
      ...
      WHERE id = auth.uid()
      AND (role IN ('admin','pasteur') OR 'communication' = ANY(groups) OR 'media' = ANY(groups))
    )
  );
